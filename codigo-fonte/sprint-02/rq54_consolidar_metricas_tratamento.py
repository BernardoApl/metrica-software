"""RQ54 (RQ3): consolida complexidade, LOC e duplicacao por tratamento.

Le o CSV consolidado da RQ30 (dados/lab02_rq30_metricas_estaticas.csv) e produz,
por tratamento (com_ia vs sem_ia) - agregando todos os katas -, a mediana e o
IQR de complexidade ciclomatica media, LOC e percentual de duplicacao. Usa
mediana/IQR em vez de media/desvio-padrao, conforme a recomendacao de robustez
estatistica do LAB02 para o N pequeno do desenho within-subject (ver RQ33 -
Hipoteses). Complementa a RQ44, que agrupa por kata e tratamento; aqui o
agrupamento e somente por tratamento, para uma visao geral da RQ3.
"""
...
SAIDA_CSV_PADRAO = Path(__file__).resolve().parents[2] / "dados" / "lab02_rq54_consolidacao_tratamento.csv"
SAIDA_GRAFICO_PADRAO = Path(__file__).resolve().parents[2] / "dados" / "rq54_consolidacao_tratamento.png"
...
def consolidar_por_tratamento(registros: list[dict]) -> list[dict]:
    """Uma linha por tratamento, agregando todos os katas, com mediana/IQR/N de cada metrica."""
    linhas = []
    por_tratamento = agrupar(registros, lambda r: r["tratamento"])
    for tratamento, grupo in sorted(por_tratamento.items()):
        katas = sorted({r["kata"] for r in grupo})
        linha = {"tratamento": tratamento, "n_trials": len(grupo), "n_katas": len(katas)}
...
def gerar_grafico(linhas: list[dict], caminho: Path) -> None:
    ...
    for eixo, metrica in zip(eixos, METRICAS):
        ...
        for tratamento in tratamentos:
            correspondentes = [l for l in linhas if l["tratamento"] == tratamento]
            valor = correspondentes[0][f"{metrica}_mediana"] if correspondentes else None
            valores.append(valor if valor is not None else 0)
        eixo.bar(tratamentos, valores, color=["#4c72b0", "#dd8452"][: len(tratamentos)])
...
def principal(argv=None) -> int:
    ...
    linhas = consolidar_por_tratamento(registros)
    salvar_csv(linhas, args.saida_csv)

    for linha in linhas:
        print("%s | n_trials=%d | n_katas=%d | CC mediana=%s (IQR=%s) | LOC mediana=%s (IQR=%s) | Duplicacao mediana=%s%% (IQR=%s)" % (
            linha["tratamento"], linha["n_trials"], linha["n_katas"],
    ...
    print("Consolidacao salva em: %s" % args.saida_csv.resolve())
    return 0
